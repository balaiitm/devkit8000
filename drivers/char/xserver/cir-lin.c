...
void check_data(void)
{
  ...
  /*
      Get the keypress - down and up event and send to userspace
      this will help the userspace application to handles .....
  */
  
  //  printk(" Check Data userdata %d\n",user_data);
  if (user_data==302459)
    {
      bound_data=458832;      
      user_data=left;
  ...
  else  if (user_data==302376)
    {
      printk("IR Received  left alt");
     bound_data=user_data; // Needs to be mapped with android value later
     user_data=leftalt;
...
static int __init cir_init (void)
{
  ...
  //for(dev=100;dev<300&&ret<0;dev++) {
  //	printk("Trying to set Major Number %d\n",dev);
 // 	ret = register_chrdev (dev, CIR_DEVICE, &cir_fops);
  //}
  ret = register_chrdev (CIR_MAJOR, CIR_DEVICE, &cir_fops);
  if (ret < 0)
    {
        
      printk (KERN_ERR CIR_DEVICE ": register_chrdev failed! Error = %d \n",ret);
      return -EIO;
    }
  // omap_cfg_reg(irq);
