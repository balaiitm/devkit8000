...
static const u8 twl4030_reg[TWL4030_CACHEREGNUM] = {
	...
	0x0c, /* REG_AVTXL2PGA		(0xC)	*/
	0x0c, /* REG_AVTXR2PGA		(0xD)	*/
...
static const u8 Oldtwl4030_reg[TWL4030_CACHEREGNUM] = {
	0x00, /* this register not used		*/
...
static void twl4030_init_chip(struct snd_soc_codec *codec)
{
	...
	twl4030_write(codec, TWL4030_REG_HS_POPN_SET, popn);
 	ret = twl4030_read_reg_cache(codec, TWL4030_REG_OPTION);
	if (ret < 0)
		goto enable_ip_exit;

	opt_ip = (ret &  (~0x0f));

	/* enable Aux-L and Aux-R i/p amplifiers */
  opt_ip |= 0x3;

    /* select auxilliary inputs for audio in Left */
    mic_en1 = 0x11;
    /* select auxilliary inputs for audio in Right */
    mic_en2 = 0x0;
    /* power control enable */
    adc = 0x0A;

    ret = twl4030_write(codec, TWL4030_REG_OPTION, opt_ip);
    if (ret) {
        line = __LINE__;
        goto enable_ip_exit;
    }
    ret = twl4030_write(codec, TWL4030_REG_ANAMICL, mic_en1);
    if (ret) {
        line = __LINE__;
        goto enable_ip_exit;
    }
    ret = twl4030_write(codec, TWL4030_REG_ANAMICR, mic_en2);
    if (ret) {
        line = __LINE__;
        goto enable_ip_exit;
    }
    ret = twl4030_write(codec, TWL4030_REG_MICBIAS_CTL, 0x1);
    if (ret) {
        line = __LINE__;
        goto enable_ip_exit;
    }
    ret = twl4030_write(codec, TWL4030_REG_AVADC_CTL, adc);
    if (ret) {
        line = __LINE__;
        goto enable_ip_exit;
    }
    /* Set off digital mics and select ADC o/p as i/p to Tx 1*/
    ret = twl4030_write(codec, TWL4030_REG_ADCMICSEL, 0x0);
    if (ret) {
        line = __LINE__;
        goto enable_ip_exit;
    }

    /* set gain <= 6dB on  Aux input */
    ret = twl4030_write(codec, TWL4030_REG_ANAMIC_GAIN, 0x24);
    if (ret) {
        line = __LINE__;
        goto enable_ip_exit;
    }

    /* Set Tx path volume control as 12dB */
    ret = twl4030_write(codec, TWL4030_REG_ATXL1PGA, 0x18);
    ret = twl4030_write(codec, TWL4030_REG_ATXR1PGA, 0x18);
    ret = twl4030_write(codec, TWL4030_REG_AVTXL2PGA, 0x18);
    ret = twl4030_write(codec, TWL4030_REG_AVTXR2PGA, 0x18);

    ret = twl4030_write(codec, TWL4030_REG_DIGMIXING, 0x0);  /* No Karaoke */
enable_ip_exit:
    if (ret)
